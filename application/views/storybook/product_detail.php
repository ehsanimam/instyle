	...
	*/
	...
	<tr>
		...
		else
		{
			...
			$url3 = $this->set->get_subsubcaturl($product->subsubcat_id);
			$row3 = $url3->row_array();
			$ssc_url_structure = $row3['url_structure'];

			// breadcrumbs
			echo anchor($c_url_structure, $product->cat_name).nbs(2).'/'.nbs(2); // ----> bread 1 - category
			
			if ($this->config->item('site_domain') === 'www.yourdomain.com') // ---> where subsubcat is present
			{
				echo anchor($c_url_structure, $product->designer).nbs(2).'/'.nbs(2); // ----> bread 2 - still category
				echo anchor($c_url_structure.'/'.$sc_url_structure, $product->subcat_name).nbs(2).'/'.nbs(2); // ----> bread 3 - subcat
				echo anchor($c_url_structure.'/'.$sc_url_structure.'/'.$ssc_url_structure, $product->subsubcat_name).nbs(2).'/'.nbs(2); // ----> bread 3 - subcat
			}
			else if ($this->config->item('site_domain') === 'www.storybookknits.com') // ---> where hub type is for only one designer
			{
				echo anchor($c_url_structure, $product->designer).nbs(2).'/'.nbs(2); // ----> bread 2 - still category
				echo anchor($c_url_structure.'/'.$sc_url_structure, $product->subcat_name).nbs(2).'/'.nbs(2); // ----> bread 3 - subcat
			}
	...
	*/
		...
		<td rowspan="2" style="position:relative;width:340px;height:510px;padding: 0 15px 3px 0;">
			...
			*/
			...
			if ($this->config->item('site_domain') === 'www.yourdomain.com')
				$img_path 			= 'product_assets/'.$product->c_folder.'/'.$product->d_folder.'/'.$product->sc_folder.'/'.$product->ssc_folder.'/';
			else
				$img_path 			= 'product_assets/'.$product->c_folder.'/'.$product->d_folder.'/'.$product->sc_folder.'/';
			$img_name 			= $product->prod_no.'_'.$color_code;
			
			$img_large			= $this->config->item('PROD_IMG_URL').$img_path.'product_front/'.$img_name.'.jpg';
			$img_thumb			= $this->config->item('PROD_IMG_URL').$img_path.'product_front/thumbs/'.$img_name.'_3.jpg';
			
			$img_front			= $this->config->item('PROD_IMG_URL').$img_path.'product_front/thumbs/'.$img_name.'_2.jpg';
			$img_side			= $this->config->item('PROD_IMG_URL').$img_path.'product_side/thumbs/'.$img_name.'_2.jpg';
			$img_back			= $this->config->item('PROD_IMG_URL').$img_path.'product_back/thumbs/'.$img_name.'_2.jpg';
			
			$img_front_thumb	= $this->config->item('PROD_IMG_URL').$img_path.'product_front/thumbs/'.$img_name.'_3.jpg';
			$img_side_thumb		= $this->config->item('PROD_IMG_URL').$img_path.'product_side/thumbs/'.$img_name.'_3.jpg';
			$img_back_thumb		= $this->config->item('PROD_IMG_URL').$img_path.'product_back/thumbs/'.$img_name.'_3.jpg';

			$img_front_large	= $this->config->item('PROD_IMG_URL').$img_path.'product_front/'.$img_name.'.jpg';
			$img_side_large		= $this->config->item('PROD_IMG_URL').$img_path.'product_side/'.$img_name.'.jpg';
			$img_back_large		= $this->config->item('PROD_IMG_URL').$img_path.'product_back/'.$img_name.'.jpg';
				...
				if ($get_color_list->num_rows() > 0)
				{
					foreach ($get_color_list->result() as $color)
					{
						...
						<div class="mouseoverdiv" id="<?php echo $id1; ?>" style="<?php echo $style; ?>">
							<div style="position:relative;">
								<?php
								$image = $this->config->item('PROD_IMG_URL').$img_path.'product_front/thumbs/'.$product->prod_no.'_'.$color->color_code.'_3';
						...
						<div class="mouseoverdiv" id="<?php echo $id5; ?>" style="<?php echo $style; ?>">
							<div style="position:relative;">
								<?php
								$image = $this->config->item('PROD_IMG_URL').$img_path.'product_side/thumbs/'.$product->prod_no.'_'.$color->color_code.'_3';
						...
						<div class="mouseoverdiv" id="<?php echo $id6; ?>" style="<?php echo $style; ?>">
							<div style="position:relative;">
								<?php
								$image = $this->config->item('PROD_IMG_URL').$img_path.'product_back/thumbs/'.$product->prod_no.'_'.$color->color_code.'_3';
				...
				<?php
					if ($get_color_list->num_rows() > 0)
					{
						foreach ($get_color_list->result() as $color)
						{
							...
							echo anchor($this->uri->segment(1).'/'.$this->uri->segment(2).'/'.str_replace(' ','-',strtolower(trim($color->color_name))).'/'.$this->uri->segment(4), img(array('src'=>$this->config->item('PROD_IMG_URL').$img_path.'product_coloricon/'.$product->prod_no.'_'.$color->color_code.'.jpg','width'=>'20','style'=>$swatch_style)), array('onmouseover'=>$java5,'onmouseout'=>$java6)).nbs(7);
						}
					}
				?>
				<br /><br />
			...
			<div id="sizetx">
												...
												{ 
													if ($this->config->item('site_domain') === 'www.storybookknits.com')
													{
														switch ($size->size_name)
														{
															...
															case '16': $size_name = ''; $no_size = 'style="display:none;"'; break;
															default: $size_name = $size->size_name;
														}
														?>
														<option value="<?php echo $size->size_name; ?>" <?php echo isset($no_size) ? $no_size : ''; ?>>
															Size <?php echo $size_name; ?>
														</option>
														<?php
													}
													else
				...
				*/
				...
				<div id="dialog" title="SIZE CHART" style="display:none;">
					...
					if ($product->cat_id <> 19)
					{
						$this->load->view($this->config->slash_item('template').'size_chart');
					}
					?>
					</p>
				</div>
	...
	<tr>
		<td style="vertical-align:bottom;height:90px;">
			...
			*/
			if ($img = @GetImageSize($this->config->item('PROD_IMG_URL').$img_path.'product_front/thumbs/'.$img_name.'_2.jpg'))
			...
			if ($img = @GetImageSize($this->config->item('PROD_IMG_URL').$img_path.'product_side/thumbs/'.$img_name.'_2.jpg'))
			{ 
			...
			if ($img = @GetImageSize($this->config->item('PROD_IMG_URL').$img_path.'product_back/thumbs/'.$img_name.'_2.jpg'))
			{ 
			...
			if (get_http_response_code($this->config->item('PROD_IMG_URL').$img_path.'product_video/'.$img_name.'.flv') === '200' && $runway_mode == FALSE)
			{ ?>
				...
				<script type="text/javascript">
					flowplayer('flow_player', {src: "<?php echo base_url(); ?>jscript/flowplayer/flowplayer-3.2.7.swf", wmode: "opaque"}, {
						clip: {
							url: "<?php echo $this->config->item('PROD_IMG_URL').$img_path.'product_video/'.rtrim($img_name,'.jpg').'.flv'; ?>",
...
<!--HTML popup for prod zoom intro using jscript alwaysOnTop.init-->
<div id="prod_detail_intro" style="display:none;">
