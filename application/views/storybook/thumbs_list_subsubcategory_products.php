...
if ($view_pane_sql->num_rows() > 0)
{
	...
	*/
	...
	{ ?>
		<div style="width:141px;float:left;margin:2px 0px 15px 15px;">
			<table border="0" cellspacing="0" cellpadding="0" width="100%">
				<tr><td colspan="2" style="height:210px;">
					...
					/*
					| ---------------
					| Changing images to preset thumbs instead of using res.php
					...
					$img_video 		= $img_url.'product_video/'.$thumb->prod_no.'_'.$primary_img_id.'.flv';
					*/
					
					// assign the primary images
					if ($this->config->item('site_domain') === 'www.storybookknits.com')
						$img_url = 'product_assets/'.$thumb->cat_folder.'/'.$thumb->folder.'/'.$thumb->subcat_folder.'/'.$thumb->ssc_folder.'/';
					else
						$img_url		= 'product_assets/'.$thumb->cat_folder.'/'.$thumb->folder.'/'.$thumb->subcat_folder.'/';
					$img_thumb 	    = $img_url.'product_front/thumbs/'.$thumb->prod_no.'_'.$primary_img_id.'_1.jpg';
					$img_thumb_back = $img_url.'product_back/thumbs/'.$thumb->prod_no.'_'.$primary_img_id.'_1.jpg';
					$img_thumb_side = $img_url.'product_side/thumbs/'.$thumb->prod_no.'_'.$primary_img_id.'_1.jpg';
					$img_video 		= $img_url.'product_video/'.$thumb->prod_no.'_'.$primary_img_id.'.flv';
					
					// check images and set default logo where necessary
					$local_path = 'D:\www\joetaveras\milan\\';
					$img = ENVIRONMENT == 'development' ? @GetImageSize($local_path.$img_thumb) : @GetImageSize($this->config->item('PROD_IMG_URL').$img_thumb);
					$img2 = ENVIRONMENT == 'development' ? @GetImageSize($local_path.$img_thumb_back) : @GetImageSize($this->config->item('PROD_IMG_URL').$img_thumb_back);
					$img3 = ENVIRONMENT == 'development' ? @GetImageSize($local_path.$img_thumb_side) : @GetImageSize($this->config->item('PROD_IMG_URL').$img_thumb_side);
					...
					{
						$thumbnail  = $this->config->item('PROD_IMG_URL').'images/instylelnylogo_1.jpg';
						$back  		= $this->config->item('PROD_IMG_URL').'images/instylelnylogo_1.jpg';
					}
					...
					*/
					...
					<div class="fadehover" style="width:140px;height:210px;">
						...
						*/
						?>
						<!--placeholder for product images with fadehover effect (<?php echo $img_thumb; ?>-->
						...
						// This part is the old code using res.php to resize image and display it.
		...
		<?php
		/*
		// This part is for displaying a runway clip underneath the image thumbs for "View Runway Mode" option.
