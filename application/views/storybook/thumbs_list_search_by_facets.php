			...
			<table border="0" cellspacing="0" cellpadding="0" width="100%">
				<tr><td colspan="2" style="height:210px;">
					...
					*/
					...
					if ($img = @GetImageSize($this->config->item('PROD_IMG_URL').$img_thumb))
					{
						$thumbnail = $this->config->item('PROD_IMG_URL').$img_thumb;
						if ($img2 = @GetImageSize($this->config->item('PROD_IMG_URL').$img_thumb_back))
						{
							$back = $this->config->item('PROD_IMG_URL').$img_thumb_back;
						}
						else
						{
							if ($img3 = @GetImageSize($this->config->item('PROD_IMG_URL').$img_thumb_side))
							{
