<div id="header_wrapper">
	<div id="header">
		...
		<div id="logo">
			<?php echo anchor(site_url(),img(array('src'=>base_url().'images/instyle_logo.jpg','alt'=>$this->config->item('site_name')))); ?>
		</div>
		...
		<div id="mainmenu">
			<?php
				...
				*/
				...
				echo anchor(str_replace('https','http',site_url('apparel-1.html')),'WOMENS APPAREL','class="mm_first '.$link_style.'"');
				...
				*/
				if ($this->session->userdata('user_cat') != 'wholesale')
				{
					...
					echo anchor(str_replace('https','http',site_url('jewelry-19.html')),'ACCESSORIES','class="'.$link_style.'"');
				}
				...
				if ($this->session->userdata('user_cat') == 'wholesale')
				{
					...
					*/
					...
					echo anchor(str_replace('https','http',site_url('clearance/cocktail-dresses-75.html')),'CLEARANCE','class="'.$link_style.'"');

					/*
					|-----------------------------------------
					| LOGOUT - for wholesale
					*/
					echo anchor(str_replace('https','http',site_url('sign_out.html')),'LOGOUT','style="color:red;"');
				}
				...
				*/
				if ($this->session->userdata('user_cat') != 'wholesale')
				{
					$link_style = uri_string() == 'page/register' ? 'active' : '';
					$link2 = isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] == 'on' ? site_url('register') : str_replace('http','https',site_url('register'));
					echo anchor($_SERVER['SERVER_NAME'] === 'localhost' ? site_url('register') : $link2,'REGISTER','class="'.$link_style.'"');
				}
