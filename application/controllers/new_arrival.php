...
class New_arrival extends CI_Controller {
	...
	function index()
	{
		...
		$des 		= explode('-',rtrim($this->uri->segment(1),'.html'));
		...
		if(in_array('designer',$des)) {	
			...
			if($this->set->ifpaging($uri_subcat)) {
				$uri_subcat = '';
			} else {
			...
			if($this->set->ifpaging($uri_subsubcat)) {
				$uri_subsubcat = '';
			} else {
		...
		if ($show_product_detail)
		{
			...
			$site_title			= $prod_query->prod_no.' &nbsp; '.str_replace('-',' - ',ucfirst($facet_colors).'-'.ucfirst($facet_styles).' '.$prod_query->subcat_name);
		...
		elseif($uri_des && $uri_designer && $uri_subcat) 
		{
			...
			*/
			...
			$subcat 		= $this->query_category->get_designers_newarrival();
		...
		elseif($uri_des)
		{
			...
			*/
			...
			$subcat 		= $this->query_category->get_designers_newarrival();
			$meta 			= $this->query_category->get_random_meta();
			$meta 			= $meta->row();
		...
		elseif(!$uri_des && $uri_cat && $uri_subcat)
		{
			...
			*/
			...
			$subcat 		= $this->query_category->get_newarrival();
			$meta 			= $this->query_category->get_meta('tblsubcat',array('subcat_id'=>$subcat_id));
			$meta 			= $meta->row();
