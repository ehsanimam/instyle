...
class Products extends CI_Controller {
	...
	function index() {
		...
		if($this->uri->segment(3) || $this->uri->segment(4)) {
			$item_segment3 = $this->uri->segment(3);
			$item_prod3 = explode('-',$item_segment3);
			
			$item_segment4 = $this->uri->segment(4);
			$item_prod4 = explode('-',$item_segment4);
			
			if($item_prod3[0]=='item' || $item_prod4[0]=='item') {
				
				if($item_prod3[0]=='item') {
					$product_id = substr($item_prod3[1],1);
				} elseif($item_prod4[0]=='item') {
					$product_id = substr($item_prod4[1],1);
				}
		...
		if($show_product_detail) {
			...
			$uri_count 	= count($url)-1;
			
			$url 		= explode('-',$url[$uri_count]);
			$color_code = substr($url[1],1,-5);
	...
	function search() {
		...
		if($this->input->post('cat_id')) {		
			...
			$all_colors		 = $this->input->post('all_colors');
			$all_styles		 = $this->input->post('all_styles');
			$all_prices		 = $this->input->post('all_prices');
			...
			if($all_colors) {
				$this->session->set_userdata('faceted_all_colors',true);
			} else {
				$this->session->set_userdata('faceted_all_colors',false);
			}
			
			if($all_styles) {
				$this->session->set_userdata('faceted_all_styles',true);
			} else {
				$this->session->set_userdata('faceted_all_styles',false);
			}
			
			if($all_prices) {
				$this->session->set_userdata('faceted_all_prices',true);
			} else {
				$this->session->set_userdata('faceted_all_prices',false);
			}
			
			if($colors) {
				foreach($colors as $color) {
					$keywords 		.= $color.' ';
					$faceted_colors .= $color.'-';
				}
				$this->session->set_userdata('faceted_colors',$faceted_colors);
			}

			if($styles) {
				foreach($styles as $style) {
					$keywords 		.= $style.' ';
					$faceted_colors .= $style.'-';
				}
				$this->session->set_userdata('faceted_styles',$faceted_colors);
			}
			
			if($price) {
				$this->session->set_userdata('faceted_price',$price);
			}
			
			if($designer) {
				$this->session->set_userdata('faceted_designer',$designer);
			}
			
			$data = array('faceted_cat_id'		=> $cat_id,
						  'faceted_subcat_id'	=> $subcat_id,
						  'faceted_keywords'	=> $keywords
						  );
			$this->session->set_userdata($data);		
		}
		
		$cat_id 		 = $this->session->userdata('faceted_cat_id');
		$subcat_id 	 	 = $this->session->userdata('faceted_subcat_id');
		$search_terms	 = $this->session->userdata('faceted_keywords');
		...
		$load_query 	= $this->query_product->get_search_products($cat_id, $subcat_id, $this->session->userdata('faceted_price'), $this->session->userdata('faceted_designer'), $search_terms, $num, $this->offset);
		$count			= $this->query_product->get_search_products_count($cat_id, $subcat_id, $this->session->userdata('faceted_price'), $this->session->userdata('faceted_designer'), $search_terms)->num_rows();
		...
		$uri_subcat			= isset($isSubcat) ? $isSubcat[0].'.html' : 'html';
		$paging_url			= base_url().'search-results/'.substr($uri_subcat,0,-5).'/';
