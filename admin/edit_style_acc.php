<?php
	...
	if(@$action=='delete')
	{
	 $delete="delete from tblstyle where style_id='$eid' AND heading_id = '19'";
	 mysql_query($delete);
	...
	if(empty($_GET['act']) == false){
		$sql_ = "select * from tblstyle where style_id='".$_GET['pid']."' AND heading_id = '19'";
		...
		$sql_up = "update tblstyle set view_status='$val' where style_id='".$_GET['pid']."' AND heading_id = '19'";
		$res_up= mysql_query($sql_up);
	}

	$select="select * from tblstyle WHERE heading_id = '19' order by style_name";
...
<tr><td height="333" class="tab" align="center" valign="middle">
	...
	<!--bof form=================================================================================-->
	<form name="faq" action="edit_style_acc.php" method="post">
<table width=70% border=1 bordercolor=cccccc cellspacing=0 cellpadding=0>
<tr><td>
	<table width=100% align=center cellspacing=2 cellpadding=2 >
	<tr bgcolor=cccccc>
		<td align="center" colspan="2"><h1>Edit/Delete Styles Facet for <span style="color:red;">Jewelries And Accessories</span></h1></td></tr>
          <tr bgcolor=cccccc>
		...
		<?php
			 while($row=mysql_fetch_array($result)){
			  $sql_ = "select * from `tblstyle` where `style_id`='".$row['style_id']."' AND heading_id = '19'";
			  $res_ = mysql_query($sql_);
		...
		<tr align=center>
			...
			<td width="59%" align="left">
				<span class="text">[</span><a href="#" onclick="javascript:window.open('style_edit_popup_acc.php?eid=<?php echo $row[style_id];?>','','height=700 width=520 scrollbars')" class="pagelinks">edit</a><span class="text">]</span>
				<span class="text">[</span><a href="edit_style_acc.php?eid=<?php echo $row[style_id];?>&action=delete" onclick="return confirm_delete()" class="pagelinks">delete</a><span class="text">]</span>
