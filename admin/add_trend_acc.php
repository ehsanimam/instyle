<?php
	...
	if(@$action=='add')
	{
		...
		{
			  $cid=0;
			  $cid = admin_add_trend($name,$cat_image,$priority,$heading_id,$title,$description,$keyword,$alttags,$url_structure);
			if($cid != -1){
				$err="trend has been added.";
			}else {
				$err = "trend already exists"; 
			$name='';
			}
		}
		else
...
{
    document.trend.method="post";
    document.trend.action="add_trend_acc.php?action=add";
    document.trend.submit();
    
}


function check_file() {
	var fileVal=document.trend.pdF_price.value;
	var fileVal_2=document.trend.pdF_color.value;
	if(fileVal!="") {
			var ext_p=fileVal.substr(fileVal.lastIndexOf(".")).toLowerCase();
			if(ext_p!=".pdf") {
				alert("This is not a PDF file ! Please verify    ");
				document.trend.pdF_price.focus();
				return false;
			}	
		}
	...
	if(fileVal_2!="") {
			...
			if(ext_c!=".pdf") {
				alert("This is not a PDF file ! Please verify    ");
				document.trend.pdF_color.focus();
				return false;
			}	
		}
...
<tr><td height="333" class="tab" align="center" valign="middle">
	...
	<!--bof form=================================================================================-->
	<form name="trend" action="add_trend_acc.php?action=add" method="post" enctype="MULTIPART/FORM-data" onsubmit="javascript: return check_file();">
                    <table width=90% border=1 bordercolor=cccccc cellspacing=0 cellpadding=0><tr><td>

                    <table width=100% align=center cellspacing=2 cellpadding=2>
                    <tr bgcolor=cccccc><td align=center colspan=2><h1>ADD Trend Facet For <span style="color:red;">Jewelries And Accessories</span></h1></td></tr>
                    <tr><td align=center colspan=2 class="error"><? echo @$err; ?></td></tr>

                    <tr>
						<td width="45%" class="text" align="right">Trend Name : </td>
                 ...
                 <tr> 
                  ...
                  <td  align="left"><input type="text" name="url_structure"  class="inputboxbig" value="<?=@$url_structure;?>"></td>
					...
					<tr>
                      <td valign=top align=right class="text" >Priority (affects the order in which trend is displayed 1-127)</td>
