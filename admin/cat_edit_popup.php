<?php
	...
	if(@$action=='edit')
	{
		if($name!='')
		{
			...
			$update_query="update tblcat set cat_name='$name',def_subcat_id='$subcat',priority='$priority', heading_id='$heading_id',title='".addslashes($title)."',description='".addslashes($description)."',keyword='".addslashes($keyword)."',alttags='".addslashes($alttags)."',url_structure='".addslashes($url_structure)."',footer='".addslashes($footer_text)."' where cat_id='$eid'";
			mysql_query($update_query);
			if ($cat_image!='')
			{
				admin_newcatimg($cat_image, $eid);
			...
			if (!empty($strTempFile1_pdf_p))
			{
				...
				$sql_pdf_p="update tblcat set pdf_p='$strFile1_pdf_p' where cat_id ='$eid'";
			...
			if (!empty($strTempFile1_pdf_c))
			{
				...
				$sql_pdf_c="update tblcat set pdf_c='$strFile1_pdf_c' where cat_id ='$eid'";
			...
			print "<script>opener.location.href='edit_category.php';window.close();</script>";
		}
		else
		{
	...
	$select = "SELECT * FROM tblcat WHERE cat_id = '".$eid."'";
	...
	if (@$name == '')
	{
		...
		$imgn = $row['cat_img'];
		$imgs = $row['img_name'];
		$def_subcat_id = $row['def_subcat_id'];
...
<!--bof form==============================================================-->
<form name="category" action="cat_edit_popup.php?eid=<?=$eid;?>&action=edit" method="post" enctype="MULTIPART/FORM-data">
...
<tr><td>
	...
	<table width="100%" align="center" cellspacing="2" cellpadding="2">
		...
		<tr> 
            <td width="460" valign=top class="text" align="right">Category Name : </td>
            <td width="413">
		...
		-->
		<tr> 
            <td width="460" valign=top class="text" align="right">Default SubCategory Name : </td>
            <td width="413">
				<?php
				...
				<select name="subcat">
					...
					{ ?>
						<option value="<?=$rst['subcat_id']?>" <? if($rst['subcat_id']==$def_subcat_id){ ?> selected <? }?>><?=$rst['subcat_name']?></option>
						<?php
					} ?>
				</select>
			</td>
		...
		<tr>
            <td align=center colspan="2" class="error">
				<?php
				if ($imgn == '') echo 'No uploaded image icon.';
				else
				{ ?>
					<img src="../images/category/<?=$imgn;?>" />
					<?php
				} ?>
			</td>
		</tr>
		...
		<tr> 
            <td height="30" colspan="2" align="center" valign="top"><span class="text" style="color:#FF0000">(image size should be 7500(w) x568(h) px)</span></td>
		</tr>
		<tr>
