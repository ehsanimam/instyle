<?php
	...
	if(@$action=='edit')
	{
		if($name!='')
		{
			$update_query="update tblstyle set style_name='$name',priority='$priority', heading_id='$heading_id',title='".addslashes($title)."',description='".addslashes($description)."',keyword='".addslashes($keyword)."',alttags='".addslashes($alttags)."',url_structure='".addslashes($url_structure)."' where style_id='$eid'";
			...
			*/
			...
			if (!empty($strTempFile1_pdf_p))
			{
				...
				$sql_pdf_p="update tblstyle set pdf_p='$strFile1_pdf_p' where style_id ='$eid'";
 
				mysql_query($sql_pdf_p) or die("Error updating tblstyle");
			}
			//********************************************************
			//******************************************* Color PDF
			...
			if (!empty($strTempFile1_pdf_c))
			{
				...
				$sql_pdf_c="update tblstyle set pdf_c='$strFile1_pdf_c' where style_id ='$eid'";
			...
			print "<script>opener.location.href='edit_style_acc.php';window.close();</script>";
		}
		else
		{
	...
	$select="select * from tblstyle where style_id='$eid' AND heading_id = '19'";
...
<!--bof form=================================================================================-->
<form name="style" action="style_edit_popup_acc.php?eid=<?=$eid;?>&action=edit" method="post" enctype="MULTIPART/FORM-data">
<table width=100% border=1 bordercolor=cccccc cellspacing=0 cellpadding=0><tr><td>
<input type="hidden" name=eid value="<?=$eid;?>">
	<table width=100% align=center cellspacing=2 cellpadding=2>
          <!--DWLayoutTable-->
          <tr bgcolor=cccccc> 
            <td align=center colspan=2><h1>Edit Style Facet - <span style="color:red;">Jelweries And Accessories</span></h1></td>
          </tr>
          <tr>
          ...
          <tr> 
            <td width="460" valign=top class="text" align="right">Style Name 
              : </td>
            <td width="413">
          ...
          <tr> 
            <td width="460" valign=top class="text" align="right">Substyle Name 
              : </td>
            <td width="413">
			...
			{   ?>
				<option value="<?=$rst['substyle_id']?>" <? if($rst['substyle_id']==$def_substyle_id){ ?> selected <? }?>><?=$rst['substyle_name']?></option>
			<? }
			?>
