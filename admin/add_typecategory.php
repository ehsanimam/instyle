...
if(@$action=='add')
{
 if($type_name!='')
  {
	  $getid=admin_ins_typecat($type_name,$heading,$title,$description,$keyword,$alttags);
	  if($getid != -1)
	  {
		 $err="Type Category has been added.";
		 $type_name='';
		 $cat='';
		  header('location:edit_typecategory.php?msg=1');
	  }else
	     $err="Type Category already exists.";
...
<tr>
	...
	<form name="typecategory" action="add_typecategory.php?action=add" method="post" enctype="MULTIPART/FORM-data">
<table width=85% border=1 bordercolor=cccccc cellspacing=0 cellpadding=0><tr><td>
	<table width=100% align=center cellspacing=2 cellpadding=2>
                <tr bgcolor=cccccc>
                  <td align=center colspan=2><h1>ADD Type Sub Category</h1></td>
                </tr>
                <tr>
                ...
                <tr>
                  <td valign=top class="text" align="right">Select Category: </td>
                  <td align="left">
				  	<select name="heading" id="heading" class="inputbox" onChange="getQty('subcat2.php?option=typecat&catid='+this.value)">
						...
						<?php
						 ...
						 ?>
						 <option value="<?=$row_heading['cat_id']?>"><?php echo $row_heading['cat_name'];?></option>
						 <?php } ?>
					</select>
				  </td>
                </tr>
                 <tr> 
                  <td width="48%" align="right" valign=top class="text">Type Sub Category : </td>
                  <td width="52%" align="left"><div id="subcatdiv">Select SubCategory</div></td>
                </tr>
                 <tr> 
