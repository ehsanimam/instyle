<?php
	...
	if(@$action=='add')
	{
		if($name!='' )
		{
			$cid = admin_add_event($name, $cat_image,$priority,$heading_id,$title,$description,$keyword,$alttags,$url_structure);
			if($cid != -1)
			{
...
{
    document.event.method="post";
    document.event.action="add_event.php?action=add";
...
function check_file() {
	var fileVal=document.event.pdF_price.value;
	var fileVal_2=document.event.pdF_color.value;
	if(fileVal!="") {
			var ext_p=fileVal.substr(fileVal.lastIndexOf(".")).toLowerCase();
			if(ext_p!=".pdf") {
				alert("This is not a PDF file ! Please verify    ");
				document.event.pdF_price.focus();
				return false;
			}	
		}
	...
	if(fileVal_2!="") {
			...
			if(ext_c!=".pdf") {
				alert("This is not a PDF file ! Please verify    ");
				document.event.pdF_color.focus();
				return false;
			}	
		}
...
<tr>
	...
	<form name="event" action="add_event.php?action=add" method="post" enctype="MULTIPART/FORM-data" onsubmit="javascript: return check_file();">
                    <table width=90% border=1 bordercolor=cccccc cellspacing=0 cellpadding=0><tr><td>

                    <table width=100% align=center cellspacing=2 cellpadding=2>
                    <tr bgcolor=cccccc><td align=center colspan=2><h1>ADD Event Facet For <span style="color:red;">Womens Apparel</span></h1></td></tr>
                    <tr><td align=center colspan=2 class="error"><? echo @$err; ?></td></tr>

                    <tr>
						<td width="45%" class="text" align="right">Event Name : </td>
					...
					|
						$sql = sprintf("select id, headings from tbl_type_headings");
								echo SelectValue($sql, "heading_id", @$heading_id ? @$heading_id : 1 ); 
					*/
						?>
                 ...
                 <tr> 
                  ...
                  <td  align="left"><input type="text" name="url_structure"  class="inputboxbig" value="<?=@$url_structure;?>"></td>
                    ...
                    <tr>
                      <td valign="middle" class="text" align="right">Do you want to show this event on index page?</td>
