<?php
	...
	*/
	...
	if (isset($_GET['action']) && !isset($_GET['designer']))
	{
		...
		{
			echo '<script>
						window.location.href="order_log_retail.php";
					</script>';
		}
	// End Email...............................
	}
	// send copy of order to email address entered in order detail page
	if(isset($_GET['action']) && isset($_GET['designer']))
	{
		$trans_q = "
			SELECT 
		...
		{
			echo '<script>
						window.location.href="order_log_retail.php?order_id='.$_GET['order_id'].'&designer='.$_GET['designer'].'";
					</script>';
		}
	// End Email...............................
	}
	...
	if (isset($_POST['update_remarks']))
	{
		...
		echo '
			<script>
				window.location.href="order_log_retail.php?order_id='.$order_id.'";
			</script>
		';
	}
			...
			<table style="outline:1px solid #787878;margin: 10px 0 10px;">
				...
				<tr bgcolor=cccccc valign="middle" height="35"><td colspan="6" style="position:relative;">
					<div style="position:relative;width:100%;text-align:center;padding-bottom:5px;">
						<input type="button" value="<< Back" onclick="window.location.href='order_log_retail.php'" style="position:absolute;left:5px;top:-5px;" />
						...
						<table width="100%" border="0" cellpadding="0" cellspacing="0"  bgcolor="">
							...
							<tr><td class="text" align="center">
								...
								else
								{
									...
									echo '<a href="order_log_retail.php?action=email&order_id='.$_GET['order_id'].'" class="del_link" style="font-size:12px;">Resend to User</a>';
									echo '</div>';
								}
								?>
				...
				<tr>
					<td align="center" colspan="8"><?php echo '<a href="order_log_retail.php?order_id='.$_GET['order_id'].'" style="text-decoration:none;">'; ?>View all items on this transaction</a></td>
				</tr>
	...
	else
	{
		$query = "
			SELECT 
				...
				tlog.firstname as fname, tlog.lastname as lname, email
			FROM 
				tbl_order_log tlog
			LEFT JOIN
				tbl_order_log_details tdet ON tdet.order_log_id = tlog.order_log_id
		...
		<tr><td height="333" class="tab" align="center" valign="middle">
			...
			<table cellpadding="0" cellspacing="2" style="outline:1px solid #787878">
				...
				<tr bgcolor="cccccc" valign="middle" height="35"><td align="center" colspan="13"><h1>Order Log Retail</h1></td></tr>
				...
				<tr bgcolor="#cccccc" height="35">
					...
					<td width="40" align="center" valign="middle" class="head">Buyer Name</td>
			...
			while ($rows = mysql_fetch_array($result))
			{
				...
				<tr class="text" height="20" bgcolor="efefef" onMouseOver="this.bgColor='cccccc'" onMouseOut="this.bgColor='efefef'">
					...
					<td align="center"><?php echo '<a href="order_log_retail.php?order_id='.$rows['order_id'].'&designer='.$rows['designer'].'">'.$rows['transaction_code']; ?></a></td>
					...
					<td align="center">
						[<a href="order_log_retail.php?del=<?php echo $rows['order_id']; ?>" class="del_link" onclick="return warnme();">X</a>]
						[<a href="order_log_retail.php?action=email&order_id=<?= $rows['order_id'] ?>" class="del_link">Resend</a>]
					</td>
				</tr>
				<?php
				$i++;
