<?php
	...
	if (isset($_GET['action']) && $_GET['action'] == 'add')
	{
		...
		if (!$email || !$pword || !$firstname || !$lastname || !$address1 || !$city || !$country || !$state || !$zipcode || !$telephone)
		{
		...
		if ( ! $err)
		{
			$get_email1 = @mysql_query("SELECT * FROM tbluser_data WHERE email = '".$email."'") or die ("Select error: ".mysql_error());
			$get_email2 = @mysql_query("SELECT * FROM tbluser WHERE e_mail = '".$email."'") or die ("Select error: ".mysql_error());
			
			if (mysql_num_rows($get_email1) > 0 && mysql_num_rows($get_email2) > 0)
			{
				echo $email;
			...
			else
			{
				...
				$insert_query = "
					...
					VALUES (
						'".$email."', '".$pword."', '".$firstname."', '".$lastname."', '".$telephone."', 
						'".$dresssize."', '".$address1."', '".$address2."', '".$country."', '".$city."',
						'".$state."', '".$zipcode."', '".(int)$receive_productupd."', '".$is_active."'
					)
				";
				mysql_query($insert_query) or die ("Insert tbluser_data error: ".mysql_error());
	 
				$user_uname = $firstname.' '.$lastname;
				$insert_query = "
					INSERT INTO tbluser (
						e_mail, user_name, user_password, uname, access_level, create_date, isregistered, received_produpdate
					) 
					VALUES (
						'".$email."', '".$email."', '".$pword."', '".$user_uname."', '".$access_level."', 
						'".date('Y-m-d', time())."', '1', '".(int)$receive_productupd."'
					)
				";
						...
						<td width="72%" class=partner valign=top align=center >&nbsp;&nbsp;
							...
							<tr><td>
								...
								<table width=100% align=center cellspacing=2 cellpadding=2>
									<tr bgcolor=cccccc> 
										<td align=center colspan=2><b><font size=2 color="#000000" face="verdana,Arial">ADD User</font></td>
									</tr>
									<tr> 
										<?php
										if ( ! empty($err))
									...
									<tr> 
										...
										<td><input type="text" name="pword" class="textbox" value="instyle2011" size="30" /></td>
									</tr>
									<tr>
									...
									<tr> 
										<td height="25" class="search_head" style="font-size:11pt; font-family:Arial, Helvetica, sans-serif; text-align:right;">Dress Size :</td>
										<td align="left"><input name="dresssize" type="text" class="textbox" id="txtadd2" size="30"  value=""/></td>
									</tr>
									<tr> 
