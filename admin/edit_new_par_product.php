<?php
	...
	*/
	...
	{   
		...
		*/
		$sql = "select * from tbl_product where prod_no='".$_GET['prod_no']."'";
	...
	| -----------------------------------------------------------------------
	*/
	if (isset($_GET['act']) && $_GET['act'] == "modi")
	{
		/*
		| -----------------------------------------------------------------------
		...
		*/
			...
			$style_id1 = isset($_POST['style_id1']) ? $_POST['style_id1'] : '';
			...
			$material_id1 = isset($_POST['material_id1']) ? $_POST['material_id1'] : '';
			...
			$trend_id1 = isset($_POST['trend_id1']) ? $_POST['trend_id1'] : '';
			...
			$event_id1 = isset($_POST['event_id1']) ? $_POST['event_id1'] : '';
		...
		*/
			...
			if ($_FILES["front_".$get_img1['prod_no']]['error'] == 0 && ! empty($_REQUEST['color_name']))
			{
				...
				*/
				...
				$path06 = '../../products/'.$_POST['folder'];
				$uploadFilesTo6 = $path06.'product_front/';
				
				$img1 = explode(".",$img_front1);
				
				$image_name_1 = $get_img1['prod_no']."_".$irow['color_code'];
				$fileatt_name1 = $image_name_1.".".$img1[1];
				...
				@move_uploaded_file($fileatt1, $uploadFilesTo1.'/'.$fileatt_name1);
					// resample image
					if ($img_info = @GetImageSize($uploadFilesTo1.'/'.$fileatt_name1))
					{
						go_and_resample($img_info, 'front', $path01, $image_name_1);
					}
				// copy uploaded file to product_assets folder respective view repository
				@copy($uploadFilesTo1.'/'.$fileatt_name1, $uploadFilesTo6.'/'.$fileatt_name1);
					// resample image
					if ($img_info = @GetImageSize($uploadFilesTo6.'/'.$fileatt_name1))
					{
						go_and_resample($img_info, 'front', $path06, $image_name_1);
					}
			...
			*/
			...
			if ($_FILES["back_".$get_img1['prod_no']]['error'] == 0 && !empty($_REQUEST['color_name']))
			{
				...
				$path02 = '../'.$_POST['folder'];
				$uploadFilesTo2 = $path01.'product_back/';
				$path07 = '../../products/'.$_POST['folder'];
				$uploadFilesTo7 = $path06.'product_back/';
				
				$img2 = explode(".",$img_front2);
				
				$image_name_2 = $get_img1['prod_no']."_".$irow['color_code'];
				$fileatt_name2 = $image_name_2.".".$img2[1];
				...
				@move_uploaded_file($fileatt2, $uploadFilesTo2.'/'.$fileatt_name2);
					// resample image
					if ($img_info = @GetImageSize($uploadFilesTo2.'/'.$fileatt_name2))
					{
						go_and_resample($img_info, 'back', $path02, $image_name_2);
					}
				// copy uploaded file to product_assets folder respective view repository
				@copy($uploadFilesTo2.'/'.$fileatt_name2, $uploadFilesTo7.'/'.$fileatt_name2);
					// resample image
					if ($img_info = @GetImageSize($uploadFilesTo7.'/'.$fileatt_name2))
					{
						go_and_resample($img_info, 'back', $path07, $image_name_2);
					}
			}
			...
			*/
			...
			if ($_FILES["side_".$get_img1['prod_no']]['error'] == 0 && !empty($_REQUEST['color_name']))
			{
				...
				$path03 = '../'.$_POST['folder'];
				$uploadFilesTo3 = $path01.'product_side/';
				$path08 = '../../products/'.$_POST['folder'];
				$uploadFilesTo8 = $path06.'product_side/';
				
				$img3 = explode(".",$img_front3);
				
				$image_name_3 = $get_img1['prod_no']."_".$irow['color_code'];
				$fileatt_name3 = $image_name_3.".".$img3[1];
				...
				@move_uploaded_file($fileatt3, $uploadFilesTo3.'/'.$fileatt_name3);
					// resample image
					if ($img_info = @GetImageSize($uploadFilesTo3.'/'.$fileatt_name3))
					{
						go_and_resample($img_info, 'side', $path03, $image_name_3);
					}
				// copy uploaded file to product_assets folder respective view repository
				@copy($uploadFilesTo3.'/'.$fileatt_name3, $uploadFilesTo8.'/'.$fileatt_name3);
					// resample image
					if ($img_info = @GetImageSize($uploadFilesTo8.'/'.$fileatt_name3))
					{
						go_and_resample($img_info, 'side', $path08, $image_name_3);
					}
			}
			...
			*/
			...
			if ($_FILES["icon_".$get_img1['prod_no']]['error'] == 0 && !empty($_REQUEST['color_name']))
			{
				...
				$uploadFilesTo9 = '../../products/'.$_POST['folder'].'product_side/';
				...
				// copy uploaded file to product_assets folder respective view repository
				@copy($uploadFilesTo4.'/'.$fileatt_name4, $uploadFilesTo9.'/'.$fileatt_name4);
			}
			...
			| // ---> Resampling imagse
			*/
			// Current colors (for late image uploading and refreshing of each color on file)
			$sel_2 = "
				SELECT 
					p.prod_no,
					c.folder as cat_folder, 
					sc.folder as subcat_folder, 
				...
				WHERE p.prod_no = '".$_POST['prod_no']."'
			";
			$qry_2 = mysql_query($sel_2) or die('Select 2: '.mysql_error());
			...
			if (mysql_num_rows($qry_2) > 0)
			{
				...
				{
					$img_path = 'product_assets/'.$row_2['cat_folder'].'/'.$row_2['d_folder'].'/'.$row_2['subcat_folder'].'/';
					$img_name = $_POST['prod_no'].'_'.$row_2['color_code'];
			...
			// Additional colors (for pre uploaded images prior adding of colors)
			if (isset($_POST['color_name']) && ! empty($_POST['color_name']))
			{
				...
				$sel_4 = "
					SELECT color_code
					FROM tblcolor
					WHERE color_name = '".$_POST['color_name']."'
				";
				$qry_4 = mysql_query($sel_4) or die('Select 4: '.mysql_error());
				$row_4 = mysql_fetch_array($qry_4);

				// use $sel_2 query text to get path structure
				$qry_3 = mysql_query($sel_2) or die('Select 2b: '.mysql_error());
				
				if (mysql_num_rows($qry_3) > 0)
				{
					// need only one row
					$row_3 = mysql_fetch_array($qry_3);

					$img_path = 'product_assets/'.$row_3['cat_folder'].'/'.$row_3['d_folder'].'/'.$row_3['subcat_folder'].'/';
					$img_name = $_POST['prod_no'].'_'.$row_4['color_code'];
		...
		*/
			...
			if ($_FILES["video_".$get_img1['prod_no']]['error'] == 0 && !empty($_REQUEST['color_name']))
			{
				...
				$uploadFilesTo9 = '../../products/'.$_POST['folder'].'product_video/';
				...
				// copy uploaded file to product_assets folder respective view repository
				@copy($uploadFilesTo5.'/'.$fileatt_name5, $uploadFilesTo10.'/'.$fileatt_name5);
			}
		...
		*/
			...
			// if color is not on stock yet and color_name field is not empty, insert color and the inputted stock qty
			...
			// updating stock date for existing colors if date is added
			$sel_1 = "
				SELECT *
				FROM tbl_stock
		...
		*/
			...
			{
				$primary_img = isset($primary_img) ? $primary_img : '';
	...
	*/
	if (isset($_GET['act']) && $_GET['act'] == "del_stock")
	{
		$getp = mysql_query("SELECT
							tp.prod_no, tp.prod_id, tp.cat_id, tp.designer, tp.subcat_id,
	...
	*/
	if (isset($_GET['act']) && $_GET['act'] == "del")
	{
		...
		// ----------------------------------
		// --> delete item on new_item_stock
		mysql_query("DELETE from new_items_count WHERE prod_no = '".$prod_no."'") or die(mysql_error());
		
		// ----------------------------------
		// --> adding this uri string to pass to csv_check.php for return to list_products.php page
		$add_uri_string = isset($_GET['p']) ? '&sel='.$sel.'&p='.$_GET['p'] : '&sel='.$sel;
		?>
		<script type="text/javascript">
			<!--
			window.location = "csv_check.php?cat_id=<?=$cat_id?>&des_id=<?=$des_id?>&subcat_id=<?=$subcat_id?>&updb=1<?php echo $add_uri_string; ?>";
			//-->
		</script>
		<?php 
		/*
					...
					*/
					...
					<td class="text">
						...
						<table border="0" cellspacing="0" style="background:#ccc;border:1px solid #999999;">
                            <tr>
								<td class="text" align="right">Select color : </td>
								<td>
								<?php
